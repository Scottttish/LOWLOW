...
const initializeApp = async () => {
    try {
        ...
        const passwordResetModel = new PasswordReset(pool);
        const mailService = new MailService();
        const authService = new AuthService(userModel, passwordResetModel, mailService);
        const authController = new AuthController(authService);

        // Создаем роутер сразу при инициализации
        authRouter = authRoutes(authController);

        console.log('✅ Сервисы аутентификации инициализированы');
        console.log('✅ Роуты аутентификации созданы');
...
// Используем authRouter напрямую (он будет null до инициализации)
app.use('/api/auth', (req, res, next) => {
    if (authRouter) {
        return authRouter(req, res, next);
    } else {
        console.error('⚠️  Попытка доступа к /api/auth до инициализации роутера');
        res.status(503).json({
            success: false,
            message: 'Сервис аутентификации не готов. Пожалуйста, подождите...'
        });
    }
});
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, () => {
            ...
            console.log('   POST /api/auth/forgot-password - Забыли пароль');
            console.log('   POST /api/auth/verify-reset-code - Проверить код восстановления');
            console.log('   POST /api/auth/reset-password - Сбросить пароль');
